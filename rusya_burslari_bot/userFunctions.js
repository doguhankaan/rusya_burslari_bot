...
const pool = mysql.createPool({
  ...
  database: 'rusya_burslari', // Bağlanılacak veritabanı
  waitForConnections: true,
  connectionLimit: 10,  // Aynı anda kaç bağlantı kullanılabilir
  queueLimit: 0
});
...
async function checkSubscription(ctx) {
  ...
  const channelUsername = '@ankara_rusevi'; 
  ...
  ctx.reply(getLocalizedMessage(ctx, "scholarshipApplicantMenu"), {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: getLocalizedMessage(ctx, "scholarshipApplicant6"), callback_data: 'SSSAI' }],
...
function SSSAI(ctx){
  userSteps[ctx.from.id] = 'SSSAI';

  const userLanguage = userSelectedLanguage[ctx.from.id] || 'tr'; 
  const urls = getUrls()[userLanguage];
  ctx.reply(getLocalizedMessage(ctx, "SSSAI"), {
    reply_markup: {
      inline_keyboard: [
...
async function blockUnwantedContent(ctx) {
  if (ctx.chat.id === supportChatID) return;
  const currentStep = userSteps[ctx.from.id]; // Kullanıcının şu anki adımını öğren
  // Sadece belirli menülerde izin verilmesi gereken içerik türlerini kontrol et
  if (currentStep === 'DuyuruMetniBekleniyor') {
    announcmentConfirmation(ctx, ctx.message.text);
    // Bu menülerde her tür içeriğe izin verebilirsin, bu yüzden fonksiyondan çık
    return;
  }
  ...
  if(currentStep === 'SSSAI'){
    await ctx.reply(getLocalizedMessage(ctx, "ludaIsThinking"));
    let answer = await askQuestion(ctx.message.text);
    answer = answer + getLocalizedMessage(ctx, "answerSuffix");
